        ...
        else
        {
            ...
            for (auto& pubkey: pubkeys_)
            {
                ...
                else
                {
                    ...
                    if (sigs[iSig].back() != SIGHASH_ALL && sigs[iSig].back() != (SIGHASH_ALL | SIGHASH_FORKID_BCO)) 
...
//LOGGER(trace) << "SignableTxIn::setTxIn: pubkeys_push_back(objects[1]) returned" << std::endl;
        ...
        // TODO: add support for other hash types.
        if (sig.empty() || (sig.back() != SIGHASH_ALL && sig.back() != (SIGHASH_ALL | SIGHASH_FORKID_BCO)) )
            throw std::runtime_error("Unsupported hash type.");
...
//LOGGER(trace) << "SignableTxIn::setTxIn: Verify signature" << std::endl;
        ...
        bytes_t sighash = tx.getSigHash(SIGHASH_ALL | SIGHASH_FORKID_BCO, nIn, txoutscript, outpointamount);
        secp256k1_key key;
...
//LOGGER(trace) << "placeholder" << std::endl;
        ...
        {
            // TODO: add support for other hash types.
            if (sigs[iSig].empty() || (sigs[iSig].back() != SIGHASH_ALL && sigs[iSig].back() != (SIGHASH_ALL | SIGHASH_FORKID_BCO)))
                throw std::runtime_error("Unsupported hash type.");
            ...
            // Verify signature.
            bytes_t sighash = tx.getSigHash(SIGHASH_ALL | SIGHASH_FORKID_BCO, nIn, redeemscript_, outpointamount);
            secp256k1_key key;
            key.setPubKey(pubkey);
            if (secp256k1_verify(key, sighash, signature))
            {
